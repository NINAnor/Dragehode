...
To see the code which uses these functions go to @sec-code
...
beregn.popstruktur <- function(utvalgt.lokalitet,lokalitetsdata,transektdata,rutedata,rutebredde=1,
                               fert="Fert.planter",veg="Veg.planter",sma="SmÃƒÂ¥planter",tot="Ant.DR",
                               forekomst_transekt="Dragehode",forekomst_avstand="Forekomst dragehode (m)",
...
plot.popstruktur <- function(popstr){
  ...
  plot(popstr$year,popstr$nTot,ylim=c(0,max(popstr$Tot.CI,na.rm=T)),type="o",main=popstr$lokalitet,xlab="ÃƒÂ…r",ylab="Antall individer",xlim=tidsrom)
  ...
  legend("topleft",c("Totalt","Fertile","Vegetative","SmÃ¥planter"),lty=c(1,1,1,1),pch=c(1,1,1,1),col=c("black","red","green","blue"),bty="n")
}
...
plot.gruppetrender <- function(lokalitetsdata,gruppevariabel,lokalitetsestimater,
  ...
  for(i in 1:ngrp)
  {
    ...
    if(i==1) ylab <- c("Antall totalt","Antall fertile","Antall vegetative","Antall smÃƒÂ¥planter")
    else ylab <- rep("",length(variabler))
    xlab <- c(rep("",length(variabler)-1),"ÃƒÂ…r")
...
ggplot.popstruktur <- function(popstr){
  ...
  p=popstr_plot%>% 
    ...
    labs(x="ÃƒÂƒr", y= "Antall individer")+
    ggtitle(title)+
    theme_classic()
