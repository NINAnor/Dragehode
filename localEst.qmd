...
Disse figurene viser estimert populasjonsstørrelse, totalt og fordelt på de tre størrelsesklassene fertile, vegetative og småplanter, for hvert år i lokalitetene som inngår i overvÂ¥kingen.
...
names(popstr)<-c( "lokalitet" ,"year",      "Fertile",     "Vegetative",      "SmÃƒÆ’Ã‚Â¥planter",      "Totalt",     
...
popstrCI=popstr %>% 
  ...
  popstrCI=popstrCI%>% 
    mutate(var=rep(c("Fertile", "Vegetative", "SmÃƒÆ’Ã‚Â¥planter", "Totalt"),(dim(popstrCI)[1]/4))) %>% 
...
popstr_plot=popstr %>% 
    ...
    pivot_longer(!year, names_to = "key", values_to = "value") %>%
    #filter(!key%in% c("Fert.CI", "Veg.CI","Sma.CI", "Tot.CI")) %>% 
    filter(key%in% c("Fertile", "SmÃƒÆ’Ã‚Â¥planter", "Totalt", "Vegetative")) %>% 
...
p=popstr_plot%>% 
   ...
   scale_colour_manual(values = c("darkred", "darkblue", "darkgrey", "darkgreen"))+
    labs(x="ÃƒÆ’Ã‚Â¥r", y= "Antall individer")+
    ggtitle(title)+
    theme_classic()+
...
plot_logR<-function(Lokal){
  ...
  stat_smooth()+
  labs(x="OvervÃ¥kingsÃ¥r",y="Log vekstrate")+
...
plotLocEst(i=37) 
plotLocEst(i=39) 
#check why Spireodden lacks data for 2023
